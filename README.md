# Hi 👋, I'm Syed Nowrose

<h3 align="center">🚀 Java Backend Developer | Full Stack Developer | Spring Boot Enthusiast</h3>

<p align="center">
<img src="https://readme-typing-svg.demolab.com?font=Poppins&size=24&duration=3000&pause=1000&center=true&vCenter=true&width=700&lines=Aspiring+Software+Engineer;Java+%7C+Spring+Boot+Developer;Full+Stack+Developer;Always+Learning+New+Technologies" />
</p>

---

## 🚀 About Me

- 🎓 B.E. Computer Science & Engineering
- 🏫 Dhaanish Ahmed Institute of Technology
- 📍 Coimbatore, Tamil Nadu
- 💻 Passionate about Java Backend Development
- 🌱 Currently learning DSA, Spring Security & System Design
- 🎯 Goal: Software Engineer | Java Full Stack Developer

---

## 💻 Tech Stack

### Languages
<p>
<img src="https://skillicons.dev/icons?i=java,c,python,js"/>
</p>

### Frontend
<p>
<img src="https://skillicons.dev/icons?i=react,html,css"/>
</p>

### Backend
<p>
<img src="https://skillicons.dev/icons?i=spring,nodejs,express"/>
</p>

### Database
<p>
<img src="https://skillicons.dev/icons?i=mysql,postgres,mongodb"/>
</p>

### Tools
<p>
<img src="https://skillicons.dev/icons?i=git,github,vscode,idea,postman"/>
</p>

---

## 📌 Featured Projects

### 📚 AI Study Planner
- React.js
- Spring Boot
- PostgreSQL
- REST APIs
- Analytics Dashboard

### 🛒 CartFlow API
- Node.js
- Express.js
- MongoDB
- JWT Authentication

### 🎯 Next Leap
Career recommendation platform built using Java & Spring Boot.

### 🤖 Topper Friend
AI-powered personalized learning platform.

---

## 🏆 Achievements

- Smart India Hackathon Participant
- AI Workshop using Python
- Java Training with Hands-on Projects
- Multiple Hackathon Prototypes

---

## 🌐 Connect With Me

- 📧 **Email:** syednowrose3@gmail.com
- 💼 **LinkedIn:** https://linkedin.com/in/syed-nowrose-969412356
- 🐙 **GitHub:** https://github.com/syednowrose

---

## 📈 Current Focus

- Java
- Spring Boot
- REST APIs
- DSA
- System Design
- Open Source

---

<p align="center">
<b>💻 "Code. Learn. Build. Repeat."</b>
</p>
